...
FIELDS = ",".join([
    "NCTId", "BriefTitle", "OfficialTitle", "BriefSummary", "Phase", "OverallStatus",
    "LastUpdatePostDate", "StartDate", "PrimaryCompletionDate", "Condition",
    "InterventionName", "LeadSponsorName", "WhyStopped", "ResultsFirstPostDate",
])
...
def parse_studies(studies: list[dict[str, Any]], source_name: str) -> list[Item]:
    ...
    for st in studies:
        ...
        meta = (f"Status: {status.get('overallStatus')}. Phase: {'/'.join(phases) or 'NA'}. "
        ...
        raw = {"nctId": nct, "status": status.get("overallStatus"), "phases": phases,
               "sponsor": sponsor, "conditions": conds, "interventions": interventions,
        ...
        items.append(Item.build(source=source_name, url=f"https://clinicaltrials.gov/study/{nct}",
                                title=f"{ident.get('briefTitle') or title} ({nct})",
                                abstract=abstract, published_at=published, raw=raw))
    return items
...
class ClinicalTrialsSource(Source):
    ...
    def fetch_page(self, params: dict[str, Any]) -> dict[str, Any]:
        return http.get_json(self.cfg["url"], params=params,
    ...
    def build_params(self, page_token: str | None = None) -> dict[str, Any]:
        ...
        if self.cfg.get("phases"):
            params["filter.advanced"] += " AND (" + " OR ".join(
                f"AREA[Phase]{p}" for p in self.cfg["phases"]) + ")"
        if page_token:

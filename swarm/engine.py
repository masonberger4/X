...
def _candidates_for_slot(
    ...
    *,
    ...
    must carry the preprint label); `hook_capped` is False for a single or long post, whose
    opener is capped by its own format rather than by the hook cap. No cell carries a link
    (rule 2)."""
...
def run_swarm(
    ...
    *,
    ...
    `formats.long_section_chars` each; a thread is the genome's slots as posts. No post
    carries a link of any kind (rule 2)."""
    ...
    # A single or long post is one post capped by its own format, not by the hook cap.
    hook_capped = shape not in (SHAPE_SINGLE, SHAPE_LONG)
